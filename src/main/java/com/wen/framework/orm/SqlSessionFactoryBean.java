...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean,
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		String dtoPackage = configuration.getVariables().getProperty("dtoPackage");
		if (hasLength(dtoPackage)) {
			String[] modelPackageArray = tokenizeToStringArray(dtoPackage,
					ConfigurableApplicationContext.CONFIG_LOCATION_DELIMITERS);
			for (String packageToScan : modelPackageArray) {
				ResolverUtil<Class<?>> resolverUtil = new ResolverUtil<Class<?>>();
				resolverUtil.find(new ResolverUtil.IsA(BaseEntity.class), packageToScan);
				Set<Class<? extends Class<?>>> typeSet = resolverUtil.getClasses();
				for (Class<?> type : typeSet) {
					if (!type.isAnonymousClass() && !type.isInterface() && !type.isMemberClass()) {
						MybatisMapperBuilder modalMapperBuilder = new MybatisMapperBuilder(configuration, type);
